...
interface PaymentDetailsSectionProps {
  ...
  isLoading: boolean;
  error: string | null;
  // Reuse these so parent changes are minimal:
  onCardChange: (e: any) => void; // Will receive PaymentElementChangeEvent
  handleSubmit: (e: React.FormEvent) => void;
  isCardComplete: boolean; // Set from PaymentElement onChange -> e.complete
  email: string;
  termsAccepted: boolean;
  onTermsAccepted: (accepted: boolean) => void;
}
  ...
  return (
    <>
      ...
      <div className="p-2 border mb-6 text-white rounded-md focus-within:border-primaryBlue">
        <PaymentElement
          // Use onChange to update completion state in the parent
          onChange={onCardChange as any}
          options={{
            layout: "tabs", // or "accordion"
            // You can theme via CSS variables globally if desired
          }}
        />
      </div>
