...
export const insertConnectedPayment = internalMutation({
  ...
  handler: async (ctx, args): Promise<Id<"connectedPayments">> => {
    ...
    return await ctx.db.insert("connectedPayments", {
      eventId,
      stripePaymentIntentId,
      email,
      totalAmount,
...
export const getTotalRevenueByOrganization = query({
  ...
  handler: async (ctx, args): Promise<GetTotalRevenueByOrganizationData> => {
    ...
    const getStats = (entries: typeof payments) => {
      ...
      const tickets = entries.reduce((sum, p) => {
        return p.ticketCounts?.reduce((s, t) => s + t.quantity, 0) || 0;
      }, 0);
    ...
    const revenueByEvent: { name: string; revenue: number }[] = Object.entries(
      revenueByEventMap
    ).map(([name, revenue]) => ({
      name,
      revenue,
    ...
    const promoterTicketMap = new Map<
      Id<"users">,
    ...
    for (const ticket of filteredTickets) {
      ...
      if (!promoterTicketMap.has(promoterId)) {
        promoterTicketMap.set(promoterId, new Map());
      }

      const typeMap = promoterTicketMap.get(promoterId)!;
      typeMap.set(typeId, (typeMap.get(typeId) || 0) + 1);
    }

    const ticketTypes = await ctx.db.query("eventTicketTypes").collect();

    const ticketTypeNameMap = new Map(ticketTypes.map((t) => [t._id, t.name]));

    const promoterBreakdown = await Promise.all(
      Array.from(promoterTicketMap.entries()).map(
        async ([promoterId, typeMap]) => {
          const promoter = await ctx.db.get(promoterId);

          const sales = Array.from(typeMap.entries()).map(
            ([eventTicketTypeId, count]) => ({
              eventTicketTypeId,
              name: ticketTypeNameMap.get(eventTicketTypeId) || "Unknown",
              count,
            })
          );
